...
def convertir_mcap_vers_json(mcap_path, output_dir, mcap_name_base):
    """
    Convertit un fichier MCAP en format JSON
    """
...
def convertir_mcap_vers_csv(mcap_path, output_dir, mcap_name_base):
    """
    Convertit un fichier MCAP en format CSV avec tableaux dans une cellule
    """
...
def traiter_dossiers_out(dossiers_out, format_choisi):
    ...
    for i, dossier_info in enumerate(dossiers_out, 1):
        ...
        output_dir = os.path.join("./lidar360_conversions", nom_dossier_sortie)
    ...
    print(f"\nFichiers de sortie dans: ./lidar360_conversions/")
    
    # Afficher la structure des fichiers créés
    if os.path.exists("./lidar360_conversions"):
        print(f"\nStructure des fichiers créés:")
        for root, dirs, files in os.walk("./lidar360_conversions"):
            level = root.replace("./lidar360_conversions", '').count(os.sep)
            indent = ' ' * 2 * level
