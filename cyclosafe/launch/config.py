...
class SerialSensor:
	...
	def __init__(self, package: str, executable: str, namespace: str, port: str, type: SensorTypeEnum,
		...
		if (port == None):
			if port_hint:
				self.port = self.resolve_port(port_hint)
		else:
			self.port = port
		self.check_port()
...
sensors_list = [
	...
	# 	description="360° lidar",
	# 	port="/dev/ttyUSB0",
	# 	transform=["--x", "0.17", "--y", "-0.185", "--z", "0.03", "--roll", "0", "--pitch", "-1.57", "--yaw", "0", "--frame-id","board", "--child-frame-id", "laser2"],
	# 	topic="scan",
	# 	color=None,
	...
	SerialSensor(
		...
		description="360° lidar",
		port="/dev/ttyUSB0",
		transform=["--x", "0.050", "--y", "-0.047", "--z", "0.02", "--roll", "0", "--pitch", "0", "--yaw", "0", "--frame-id","board", "--child-frame-id", "laser2"],
		topic="scan",
		color=None,
		parameters=[{'channel_type': 'serial',
						'serial_port': "/dev/ttyUSB0",
